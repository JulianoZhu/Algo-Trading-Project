...
Define svr to be a LinearSVR model 
define the pipeline using scaler, select scaler and the model
define c_rs using np.linspace(0.001, 1, num=8, endpoint=True)
...
param_grid =  [{'svr__epsilon': epsilon_rs, 'svr__C': c_rs}]
...
Optional:
If you move the feature building with SAR, ADX etc. to inside the pipeline
you can use random search cv to test different timeperiod values

What does a linearSVR do that a linear ridge regression cannot do?
