...
dfP['pct_rets'] = (dfP['Close'] - dfP['Close'].shift(1))/dfP['Close'].shift(1)
...
dfP['syst_cum_rets'] = (dfP['syst_rets']+1).cumprod()
dfP['mkt_cum_rets'] =  (dfP['pct_rets']+1).cumprod()

dfP[['mkt_cum_rets','syst_cum_rets']].plot(grid=True,figsize=(8,5)) #plotting returns percent cumul

start = 1 #change from 0 to 1
start_val = start
