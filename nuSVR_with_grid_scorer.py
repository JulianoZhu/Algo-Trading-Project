...
"""
INSTRUCTIONS:
At the open (=time t) we know only today's open, but we do not know today's open high low and close
Therefore: shift the high, low and close for signal construction downwards (via shift(1)), 
"""

high = df['<HIGH>'].copy().shift(1)
low = df['<LOW>'].copy().shift(1)
close = close.shift(1)


"""
INSTRUCTIONS
read RSI.ADX.SAR.pptx
read https://mrjbq7.github.io/ta-lib/func_groups/momentum_indicators.html
Use the small case close and a timeperiod of 10 to define RSI
Use the small case high, low and close and a timeperiod of 10 to define the ADX
read https://mrjbq7.github.io/ta-lib/func_groups/overlap_studies.html
Use the small case high and the low and acceleration=0.2, maximum=0.2 to define SAR
"""

n=10
df['RSI'] = ta.RSI(close.values, timeperiod=n)
df['SMA'] = close.rolling(window=n).mean()
df['Corr']= close.rolling(window=n).corr(df['SMA'])
df['SAR']=ta.SAR(high, low, acceleration=0.2, maximum=0.2)
df['ADX']=ta.ADX(high, low, close, timeperiod=n)
...
"""
INSTRUCTIONS
Define a custom scaler using FunctionTransformer
Define svr to be a NuSVR model 
define the pipeline using scaler, select scaler and the model
define c_rs using c_rs = np.linspace(0.001, 1, num=2, endpoint=True) #1 default
define nu_rs to search [1]
define gamma_rs to search "scale" or "auto"
define kernel_rs to search ["rbf"]
define the param_grid
QUESTIONS TODAY ARE NOT OPTIONAL SEE BELOW
"""
...
c_rs = np.linspace(0.001, 1, num=2, endpoint=True) 
gamma_rs = ['scale', 'auto']
kernel_rs = ['rbf']
nu_rs = [1]
...
param_grid = [{'svr__C': c_rs, 'svr__gamma': gamma_rs, 'svr__kernel': kernel_rs, 'svr__nu': nu_rs}]
...
"""
QUESTIONS (you need to turn this in)
What happens to the profits if 
instead of having the model predict the return from today's open to tomorrow's open,
you have the model predict the return from tomorrow's open to the day after tomorrow's open? 
Try it and save it under a different name and turn it in.

Optional:
If you move the featue building with SAR, ADX etc. to inside the pipeline
you can use random search cv to test different timeperiod values
